...
class Order:
    ...
    def points(self):
        """Return the points earned from this order."""
    ...
    def receipt(self):
        ...
        tier_name = self.customer.__class__.__name__.replace(
            "Customer", ""
        ).lower()
        ...
        lines.append(
            "Receipt for " + self.customer.name + " (" + tier_name + ")"
        )
        lines.append("-" * 40)
...
def create_customer(name, tier):
    """Create the correct customer subclass for a membership tier."""
    customer_classes = {
        "none": Customer,
